...
export default function Ch01Page() {
  ...
  const chapter = subject?.chapters.find((c) => c.id === "ch01-real-numbers");
  if (!chapter) notFound();
  ...
  return (
    <>
      ...
      />
      <div className="mb-8">
        <p className="text-sm text-blue-500 font-medium mb-1">Chapter 1</p>
        <h1 className="text-3xl font-bold text-gray-900 mb-1">
          실수의 체계
        </h1>
        <p className="text-sm text-gray-400 mb-3">The Real Number System</p>
        <p className="text-gray-500">실수의 공리적 정의와 완비성</p>
      </div>
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {items.map((item) => (
                  <ItemCard
                    key={item.id}
                    item={item}
                    href={`/analysis/ch01-real-numbers/${key}/${item.id}`}
                  />
                ))}
              </div>
            </section>
