...
export default function Ch03CohomologyPage() {
  const subject = getSubject("algebraic-geometry");
  const chapter = subject?.chapters.find((c) => c.id === "ch03-cohomology");
  if (!chapter) notFound();
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {items.map((item) => (
                  <ItemCard
                    key={item.id}
                    item={item}
                    href={`/algebraic-geometry/ch03-cohomology/${key}/${item.id}`}
                  />
                ))}
              </div>
            </section>
